...
const carouselStorage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    cb(
      null,
      file.originalname.substr(0, file.originalname.length - 3) + Date.now() + ".jpg"
    );
  },
});
...
router.post("/api/updateSupplierPage", upload.single("file"), async (req, res) => {
  ...
  if (req.body.facebook) {
    wh.facebook = req.body.facebook;
  }
  if (req.body.instgram) {
    wh.instgram = req.body.instgram;
  }
  if (req.body.linkedin) {
    wh.linkedin = req.body.linkedin;
  }
  if (req.body.twitter) {
    wh.twitter = req.body.twitter;
  }
  if (req.body.siteName) {
    wh.site_name = req.body.siteName;
  }
  if (req.body.google) {
    wh.google = req.body.google;
  }
  if (req.body.user_id) {
    wh.user_id = req.body.user_id;
  }
  if (file) {
    wh.logo = req.file.path.substr(10);
  }
  if (req.body.footer) {
    wh.footer = req.body.footer;
  }
  if (req.body.carousel_width) {
    wh.carousel_width = req.body.carousel_width;
  }
  if (req.body.carousel_height) {
    wh.carousel_height = req.body.carousel_height;
  }
  if (req.body.user_id) {
    wh.user_id = req.body.user_id;
  }
...
router.post("/api/uploadCarouselImages", carouselUpload.array("file", 12), (req, res) => {
  ...
  for (i = 0; i < arIndex.length; i++) {
    ...
    wh["carousel_image_" + num] = req.files[i].path.substr(10);
  }
  console.log(wh);
  ...
  db.supplier_page_info
    ...
    .then((info) => {
      if (req.files[0] && req.files[0].originalname === "left") {
        info
          .update({
            left_banner_image: req.files[0]
              ? req.files[0].path.substr(10)
              : info.left_banner_image,
            right_banner_image: req.files[1]
              ? req.files[1].path.substr(10)
              : info.right_banner_image,
          })
          .then((row) => {
            res.send(row);
          });
        ...
        info
          .update({
            right_banner_image: req.files[0]
              ? req.files[0].path.substr(10)
              : info.right_banner_image,
            left_banner_image: req.files[1]
              ? req.files[1].path.substr(10)
              : info.left_banner_image,
          })
          .then((row) => {
